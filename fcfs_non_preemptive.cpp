...
void go(){
    ...
    function<void()> find_waiting_time = [&](){
        waiting_time[0] = 0;
        cout<<"Process: "<<1<<" Arrival time: "<<arrival_time[0]<<" Burst time: "<<burst_time[0]<<" Waiting time: "<<waiting_time[0]<<endl;
        vector<int> turn_around_time(n);
        for (int i = 1; i < n; i++){
            waiting_time[i] = (arrival_time[i - 1] + waiting_time[i - 1] + burst_time[i - 1]) - arrival_time[i];
            cout<<"Process: "<<i + 1<<" Arrival time: "<<arrival_time[i]<<" Burst time: "<<burst_time[i]<<" Waiting time: "<<waiting_time[i]<<endl;
        }
    ...
    float average_waiting_time = total_waiting_time / (float)n;
